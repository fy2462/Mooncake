...
if [[ "$*" == *"-p mooncake-store-client"* ]]; then exit 6; fi
...
assert result["first_failure"]["name"] == "store-client", result
...
assert commands["store-client"]["status"] == "FAIL", result
assert commands["store-master"]["status"] == "FAIL", result
assert names.index("store-client") < names.index("store-master"), result
...
assert "transfer-engine-ffi --no-default-features --features mock" in trace, trace
assert "transfer-engine-ffi --lib" not in trace and "--skip tent::" not in trace, trace
...
python3 - "$tool_dir/../../crates/transfer-engine-ffi/Cargo.toml" "$tool_dir/../../python/Cargo.toml" "$tool_dir/../../python/src/lib.rs" <<'PY'
import pathlib
import sys
import tomllib

cargo_toml = tomllib.loads(pathlib.Path(sys.argv[2]).read_text())
...
ffi_cargo_toml = tomllib.loads(pathlib.Path(sys.argv[1]).read_text())
assert ffi_cargo_toml["features"]["mock"] == [], ffi_cargo_toml

lib_rs = pathlib.Path(sys.argv[3]).read_text()
