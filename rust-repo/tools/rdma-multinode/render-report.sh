#!/usr/bin/env bash
set -Eeuo pipefail
suite_dir=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)
# shellcheck source=lib/common.sh
source "$suite_dir/lib/common.sh"
artifact_root=${RDMA_ARTIFACT_ROOT:-/home/fy2462/workspace/tmp/mooncake/rdma-multinode}
report=${RDMA_REPORT:-$artifact_root/report.md}
standard_store_result=${STORE_STANDARD_RESULT:-$artifact_root/store-standard.result}
if [[ ! -f $standard_store_result ]]; then
    standard_store_result=$artifact_root/store.result
fi

grep -q '^PASS$' "$artifact_root/verbs.result"
grep -q '^status=PASS$' "$artifact_root/te.result"
grep -q '^protocol=rdma$' "$artifact_root/te.result"
expected_directions=a-b,b-a,b-c,c-b,c-a,a-c
grep -qx "directions=$expected_directions" "$artifact_root/te.result"
for direction in ${expected_directions//,/ }; do
    key=${direction//-/_}
    grep -Eq "^direction_${key}=PASS,.*compare=OK$" "$artifact_root/te.result"
done

json_status_is "$standard_store_result" PASS
store_json=$(cat "$standard_store_result")
resilience_result=$artifact_root/store-resilience.result
test -f "$resilience_result"
readarray -t store_evidence < <(python3 - "$standard_store_result" "$resilience_result" <<'PY'
import json
import sys

required = (
    "store_node_restart",
    "master_etcd_restart",
    "rdma_reconnect",
    "degraded_read",
    "watermark_eviction",
    "mixed_stress",
    "second_standard",
)
with open(sys.argv[1], encoding="utf-8") as stream:
    standard = json.load(stream)
evidence = standard.get("evidence", {})
segments = evidence.get("objects", {}).get("small", {}).get("remote_segments", [])
multilevel = evidence.get("multilevel", {})
fallback = multilevel.get("fallback", {})
promotion = multilevel.get("promotion", {})
if (
    standard.get("status") != "PASS"
    or evidence.get("cpp_store_loaded") is not False
    or len(set(segments)) != 3
    or fallback.get("replica_type") != "LocalDisk"
    or fallback.get("replicas") != 1
    or promotion.get("memory_replicas") != 1
):
    raise SystemExit("standard Store result lacks strict 3/1 replica evidence")

with open(sys.argv[2], encoding="utf-8") as stream:
    result = json.load(stream)
if result.get("status") != "PASS":
    raise SystemExit("resilience result is not PASS")
scenarios = result.get("scenarios", {})
if any(scenarios.get(name, {}).get("status") != "PASS" for name in required):
    raise SystemExit("a required resilience scenario is not PASS")
degraded = scenarios["degraded_read"].get("evidence", {})
if degraded.get("initial_owners") != 2 or degraded.get("remaining_owners") != 1:
    raise SystemExit("degraded Store result lacks strict 2-to-1 replica evidence")
print("three=" + ",".join(segments))
print("tier=LocalDisk fallback plus one-RDMA promotion")
print(", ".join(f"{name}=PASS" for name in required))
PY
)
[[ ${#store_evidence[@]} -eq 3 ]] || exit 1
three_replica_evidence=${store_evidence[0]}
tier_evidence=${store_evidence[1]}
resilience_evidence=${store_evidence[2]}
open_status=$(sed -n 's/^status=//p' "$artifact_root/open-rdma.result")
revision=$(cat "$artifact_root/open-rdma-revision.txt")
cat >"$report" <<EOF
# Docker Soft-RoCE multi-node validation

- Host: $(uname -a)
- Docker: $(docker --version)
- Topology: three RXE devices (A/B/C) on one manifest-owned software-RoCE bridge; three TE and three Rust Store containers share the host RDMA namespace
- Verbs: PASS, \`ib_write_bw\` over RXE A/B
- Transfer Engine: PASS, protocol=rdma, six directions: a-b, b-a, b-c, c-b, c-a, a-c; every direction reports \`compare=OK\`
- Product evidence — Rust Store standard: PASS; no C++ mooncake-store process or library used
- Product evidence — Store 3/2/1 replica profiles: PASS; \`$three_replica_evidence\`; degraded 2-to-1 read survived; \`$tier_evidence\`
- Product evidence — Rust Store standard result: \`$store_json\`
- Product evidence — Rust Store resilience: PASS; \`$resilience_evidence\`
- Open-RDMA mock: $open_status at revision \`$revision\`
- Mock evidence classification: Open-RDMA is mock-only API/driver evidence; it is not cross-node data-plane evidence
- Open-RDMA checkout preserved: true (the pre-existing dirty state was unchanged)
- Native dependency audit: Rust extension needs \`libtransfer_engine.so\` and \`libtent_shared.so\`; it does not need \`libmooncake_store.so\`

Retained logs and result files: \`verbs-*.log\`, \`te-*.log\`, \`store-*.log\`, \`store-standard.result\`, \`store-resilience.result\`, and \`open-rdma.log\` under the configured artifact root.
EOF
