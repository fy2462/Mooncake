...
device=${STORE_DEVICE:-mc-rdma-rxe-a}
metadata=${STORE_METADATA_SERVER:-10.89.10.10:2379}
master_addr=${STORE_MASTER_ADDR:-10.211.55.4:50051}
master=${STORE_MASTER_CONTAINER:-mc-rdma-master-fallback}
node_a=${STORE_NODE_A_CONTAINER:-mc-rdma-fallback-a}
node_b=${STORE_NODE_B_CONTAINER:-mc-rdma-fallback-b}
client=${STORE_CLIENT_CONTAINER:-mc-rdma-client-fallback}
...
sleep 2
for container in "$node_a" "$node_b" "$client"; do
    docker cp "$suite_dir/store-node.py" "$container:/tmp/store-node.py"
    docker cp "$suite_dir/store-e2e.py" "$container:/tmp/store-e2e.py"
done
docker exec "$node_a" python3 /tmp/store-node.py --node 10.90.0.1:12401 --device "$device" --metadata "$metadata" --master "$master_addr" --ready /artifacts/store-a.ready >"$artifact_root/store-a.log" 2>&1 &
node_a_pid=$!
docker exec "$node_b" python3 /tmp/store-node.py --node 10.90.0.1:12402 --device "$device" --metadata "$metadata" --master "$master_addr" --ready /artifacts/store-b.ready >"$artifact_root/store-b.log" 2>&1 &
node_b_pid=$!
...
docker exec "$client" python3 /tmp/store-e2e.py --node 10.90.0.1:12403 --device "$device" --metadata "$metadata" --master "$master_addr" --result /artifacts/store.result >"$artifact_root/store-client.log" 2>&1
