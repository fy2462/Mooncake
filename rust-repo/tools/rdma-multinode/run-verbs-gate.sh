...
gid_index=${VERBS_GID_INDEX:-0}
server_dev=${VERBS_SERVER_DEVICE:-mc-rdma-rxe-a}
client_dev=${VERBS_CLIENT_DEVICE:-mc-rdma-rxe-b}
...
docker exec "$server" sh -lc \
    "timeout 45 ib_write_bw -x '$gid_index' -d '$server_dev' --report_gbits" \
    >"$server_log" 2>&1 &
server_pid=$!
trap 'kill "$server_pid" 2>/dev/null || true; wait "$server_pid" 2>/dev/null || true' EXIT
sleep 2
if ! docker exec "$client" sh -lc \
    "timeout 30 ib_write_bw -x '$gid_index' -d '$client_dev' '$server_ip' --report_gbits -n 10" \
    >"$client_log" 2>&1; then
