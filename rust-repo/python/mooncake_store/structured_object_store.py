...
except Exception:  # pragma: no cover - depends on built Rust extension
    _mooncake_store = None  # type: ignore[assignment]

import msgpack as _msgpack

from .store_adapter import adapt_buffer_pool, adapt_store
...
class MooncakeBundleTransfer:
    ...
    def __init__(
        ...
        buffer_pool = adapt_buffer_pool(buffer_pool)
        self.store = store
...
def _encode_ragged_tensor_dict_values(
    ...
    for key in keys:
        ...
        payload.update({f"{key}.{name}": value for name, value in sub_payload.items()})
