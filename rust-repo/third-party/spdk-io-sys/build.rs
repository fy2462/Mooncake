...
fn main() {
    ...
    // Query SPDK through its top-level event package so pkg-config preserves
    ...
    let parser = PkgConfigParser::new().force_whole_archive([
        ...
        // SPDK 26.01 has circular static references from thread/trace back to
    ...
    // spdk_syslibs.pc contains absolute archive paths for ISA-L and
    // ISA-L-crypto. pkgconf intentionally handles only -l flags, while the
    // standard pkg-config crate translates those absolute paths into Cargo
    // link-search/link-lib directives.
    pkg_config::Config::new()
        .statik(true)
        .probe("spdk_syslibs")
        .expect("pkg-config failed for SPDK system libraries");
