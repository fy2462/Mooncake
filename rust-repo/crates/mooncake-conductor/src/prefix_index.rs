...
impl PrefixCacheTable {
    ...
    /// Generate a seed from environment or random.
    ...
    /// Lazily create context data for a (ModelContext, instanceID) pair.
    /// The seed is derived from `xxh64(additional_salt, 0)`, matching Go.
    ...
    /// Get a reference to existing context data.
    fn get_context(
        &self,
    ...
    /// Register a DP rank for a model context.
    ...
    ///
    /// Go writes parent_hash as 8 bytes (little-endian uint64), then each token_id
    /// as 8 bytes (little-endian uint32 in the first 4 bytes, 4 zero bytes remaining
    /// since Go uses a `[8]byte` buffer). This function replicates that exact byte layout.
    ...
    ///
    /// Each block of block_size tokens is hashed with the previous block's hash as parent.
    ...
    /// Process a store event: computes conductor hashes and registers them.
    ///
    ...
    pub fn process_store_event(
        ...
        if event.block_hashes.len() * event.block_size as usize != event.token_ids.len() {
            ...
            // Mooncake event: single block hash — skip for now
            return Ok(());
        }
    ...
    /// Process a remove event: removes entries from the prefix store.
    ///
    /// Lock order (matching Go): proxy_hash_mapping (write) -> prefix_store (write)
    pub fn process_remove_event(
        ...
        // Delete from proxy hash mapping
    ...
    /// Add a new prefix entry to the store.
    ...
    /// Get global view of all contexts for diagnostics.
