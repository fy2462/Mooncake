...
fn test_tenant_quota_admin_policy_lifecycle_methods() {
    let service = MasterServiceImpl::with_runtime_config(MasterRuntimeConfig {
        enable_tenant_quota: true,
        default_tenant_quota_bytes: 1000,
        tenant_quota_pool_capacity_bytes: 2000,
        ..Default::default()
    });

    assert_eq!(service.get_default_tenant_quota_policy().unwrap(), 1000);
    assert_eq!(service.set_default_tenant_quota_policy(1200).unwrap(), 1200);
